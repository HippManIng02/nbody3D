...
void move_particles(particle_t *p, const f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i++)
	...
	{ 
	 ...
	 // const f32 d_3_over_2 = pow(d_2, temp); //11 (pow, div)
	  const f32 d_3_over_2 = d_2 * sqrtf(d_2);// pow(a, 3/2) = a*pow(a, 1/2) = a*sqrtf
	  
	  const f32 d_op = 1/d_3_over_2;
	  
	  //Calculate net force: 6 FLOPs
	  fx += dx * d_op; //13 (add, div)
	  fy += dy * d_op; //15 (add, div)
	  fz += dz * d_op; //17 (add, div)
	}
